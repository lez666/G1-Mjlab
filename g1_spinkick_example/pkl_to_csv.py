...
def main(
  ...
  if isinstance(data, dict):
    if "frames" in data:
      frames = data["frames"]
      fps = data["fps"]
    elif {"root_pos", "root_rot", "dof_pos"}.issubset(data.keys()):
      # Alternate format: already split root/joints.
      ...
      raise KeyError(
        f"Unsupported dict keys in {pkl_file}: {list(data.keys())}"
      )
  else:
    frames = data.frames
    fps = data.fps
  ...
  if frames is not None:
    ...
    original_duration = root_pos.shape[0] / fps

  if frames is not None:
    print(f"Loaded motion with shape: {frames.shape}")
  else:
    print(
      "Loaded motion with shapes: "
  ...
  if frames is not None:
    ...
    print(
      f"Original duration: {original_duration:.2f} seconds ({root_pos.shape[0]} frames)"
    )
  ...
  if add_start_transition and frames is not None:
  ...
  if add_end_transition and frames is not None:
    print(
  ...
  if pad_duration > 0 and frames is not None:
  ...
  if root_rot_3d is not None:
    ...
    # root_rot is already quaternion but may be WXYZ or XYZW.
    if root_rot.shape[1] != 4:
      raise ValueError(f"Expected quaternion with 4 columns, got {root_rot.shape}")
    mean_abs = np.mean(np.abs(root_rot), axis=0)
    # Heuristic: the w component should dominate for near-identity rotations.
    if mean_abs[0] >= mean_abs[3]:
      # WXYZ -> XYZW
      quats = root_rot[:, [1, 2, 3, 0]]
      print("Detected quaternion order: WXYZ -> converted to XYZW.")
    else:
      # Assume already XYZW
      quats = root_rot
      print("Detected quaternion order: XYZW (no change).")
