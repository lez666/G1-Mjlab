...
class MotionCommand(CommandTerm):
  ...
  def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRlEnv):
    ...
    self._startup_frame = int(
      max(min(self.cfg.startup_frame, self.motion.time_step_total - 1), 0)
    )
    self._hold_steps = max(int(round(self.cfg.startup_hold_time_s / env.step_dt)), 0)
    self._hold_steps_left = torch.zeros(
  ...
  def _resample_command(self, env_ids: torch.Tensor):
    if self.cfg.sampling_mode == "start":
      self.time_steps[env_ids] = self._startup_frame
    ...
    if self.cfg.sampling_mode == "start" and self._hold_steps > 0:
      self._hold_steps_left[env_ids] = self._hold_steps
    else:
      self._hold_steps_left[env_ids] = 0
  ...
  def _update_command(self):
    hold_mask = self._hold_steps_left > 0
    if torch.any(hold_mask):
      self._hold_steps_left[hold_mask] -= 1

    active_mask = ~hold_mask
